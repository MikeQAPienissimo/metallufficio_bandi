(()=>{
  ...
  function syncButtons(){
    ...
    if(!docx||!pdf||!window.state&&typeof state==='undefined')return;
    const t=state?.type;
    if(t==='pdf'){
      docx.hidden=true;
      pdf.hidden=false;
      ...
      docx.hidden=true;pdf.hidden=true;
    }
  }
  ...
  async function buildSafePdf(){
    ...
    for(const e of edits){
      ...
      const hit=findTextBlock(items,e.original_excerpt);
      if(!hit)continue;
      const block=items.slice(hit.start,hit.end+1);
      const page=pdf.getPage(pno-1);
      ...
      const rectH=Math.max(14,yTop-yBottom);
      const maxWidth=Math.max(80,x2-x1);
      const clean=winAnsiSafe(e.replacement_text,font);
      if(!clean)continue;

      let size=Math.max(6.2,Math.min(10.5,Math.max(...heights)*.95));
      let lines=wrapText(clean,font,size,maxWidth);
      while(size>5.4 && lines.length*size*1.12>rectH){
        size-=0.35;
        lines=wrapText(clean,font,size,maxWidth);
      }
      const lineHeight=size*1.12;
      const needed=Math.max(rectH,lines.length*lineHeight+5);
      const drawBottom=Math.max(8,yTop-needed);
      ...
      for(const line of lines){
        if(y<8)break;
        page.drawText(line,{x:x1+1,y,size,font,color:rgb(.04,.04,.05)});
        y-=lineHeight;
      }
      applied++;
    }
  ...
  async function downloadPdf(){
    ...
    try{
      ...
      latestPdfBytes=new Uint8Array(bytes);latestSourceName=state.file?.name||'';
  ...
  async function reanalyzePdf(){
    ...
    try{
      let bytes=(latestPdfBytes&&latestSourceName===state.file?.name)?new Uint8Array(latestPdfBytes):await buildSafePdf();
      const name=(state.file?.name||'progetto.pdf').replace(/\.pdf$/i,'')+'_REVISIONATO.pdf';
  ...
  document.addEventListener('click',e=>{
    ...
    if(pdf&&state?.type==='pdf'){
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();downloadPdf();return;
    }
    const re=e.target.closest?.('#reanalyzeBtn');
    if(re&&state?.type==='pdf'){
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();reanalyzePdf();return;
    }
    const wrongDocx=e.target.closest?.('#downloadDocxBtn');
    if(wrongDocx&&state?.type==='pdf'){
      e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();syncButtons();return;
    }
  },true);

  const observer=new MutationObserver(()=>syncButtons());
  observer.observe(document.documentElement,{subtree:true,attributes:true,attributeFilter:['hidden','class']});
  injectFixStyle();
  syncButtons();
})();